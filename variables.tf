...
# rename to lambda_s3_events_suppressor ?
variable "lambda_s3_events_suppressor" {
  type = object({
  ...
  validation {
    condition     = alltrue([for o in var.lambda_s3_events_suppressor.security_group_egress_rules : (o.cidr_ipv4 != null || o.cidr_ipv6 != null || o.prefix_list_id != null || o.referenced_security_group_id != null)])
    error_message = "Although \"cidr_ipv4\", \"cidr_ipv6\", \"prefix_list_id\", and \"referenced_security_group_id\" are all marked as optional, you must provide one of them in order to configure the destination of the traffic."
  }
}
...
variable "lambda_package_s3_bucket_name" {
  type        = string

...
| <a name="module_suppressions_bucket"></a> [suppressions\_bucket](#module\_suppressions\_bucket) | schubergphilis/mcaf-s3/aws | ~> 0.11.0 |

## Resources

| Name | Type |
|------|------|
| [aws_cloudwatch_event_rule.s3_object_event](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/cloudwatch_event_rule) | resource |
...
| [aws_cloudwatch_event_target.s3_object_event](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/cloudwatch_event_target) | resource |
...
| [aws_lambda_permission.s3_object_event](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/lambda_permission) | resource |
| [aws_s3_bucket_notification.suppressions_bucket](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/s3_bucket_notification) | resource |
...
| <a name="input_lambda_package_s3_bucket_name"></a> [lambda\_package\_s3\_bucket\_name](#input\_lambda\_package\_s3\_bucket\_name) | The name for the S3 bucket which will be created for storing the function's deployment package | `string` | n/a | yes |
| <a name="input_suppressions_s3_bucket_name"></a> [suppressions\_s3\_bucket\_name](#input\_suppressions\_s3\_bucket\_name) | The name for the S3 bucket which will be created for storing the function's deployment package | `string` | n/a | yes |
...
| <a name="input_lambda_s3_events_suppressor"></a> [lambda\_s3\_events\_suppressor](#input\_lambda\_s3\_events\_suppressor) | Lambda Streams Suppressor settings - Supresses the Security Hub findings in response to DynamoDB streams | <pre>object({<br>    name        = optional(string, "securityhub-streams-suppressor")<br>    log_level   = optional(string, "INFO")<br>    memory_size = optional(number, 256)<br>    runtime     = optional(string, "python3.8")<br>    timeout     = optional(number, 120)<br><br>    security_group_egress_rules = optional(list(object({<br>      cidr_ipv4                    = optional(string)<br>      cidr_ipv6                    = optional(string)<br>      description                  = string<br>      from_port                    = optional(number, 0)<br>      ip_protocol                  = optional(string, "-1")<br>      prefix_list_id               = optional(string)<br>      referenced_security_group_id = optional(string)<br>      to_port                      = optional(number, 0)<br>    })), [])<br>  })</pre> | `{}` | no |
